...
public sealed class FuenteOmi : IDisposable
{
    ...
    private void AlCambiarConexion(BluetoothLEDevice aparato, object _)
    {
        ...
        // VOLVER A CONECTARSE NO ES VOLVER A ESTAR SUSCRITO. WinRT tira las suscripciones GATT
        // cuando se cae el enlace, así que un collar «conectado» puede estar completamente mudo —
        // medido el 2026-08-25 con el usuario delante de una demo: 56 minutos y cero tramas.
        //
        // RECONECTAR ES RECONSTRUIR, y el remiendo que había aquí antes NO PODÍA FUNCIONAR NUNCA.
        // Reescribía el descriptor sobre la misma característica de antes de la caída, que WinRT ya
        // había destruido. El log de la prueba del 2026-09-01 lo dice sin margen:
        //
        ...
        //
        // Se borra el remiendo en vez de arreglarlo (aprendizaje nº6) y se suelta la fuente entera:
        // quien la posee la tira y abre una nueva desde el rastreo, que es lo que hace el SDK
        // oficial de Omi con su `async with BleakClient`. La política vive en Omi.Enlace, que el
        // contrato juzga sin Bluetooth (promesa 31).
        if (Omi.Enlace.QueHacer(ahora, _huboCaida) == Omi.Accion.Reconstruir)
        {
            _huboCaida = false;
            Perder("el enlace se cayó y volvió: se reconstruye entero, porque lo de antes ya está muerto");
        }
        else if (!ahora)
        {
            _huboCaida = true;
    ...
    /// <summary>
    /// Si hubo una caída desde que se armó este enlace. Es lo único que distingue «volvió» de un
    /// aviso repetido de WinRT, que los manda en ráfaga cuando el enlace baila.
    /// </summary>
    private bool _huboCaida;
