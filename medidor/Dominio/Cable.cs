...
/// <summary>
/// Arma el cuerpo de `POST /api/v1/metrics/batch` desde lo que el spool entregó. A cada fila se le
/// inyecta su `seq` — el servidor construye con él el uid de idempotencia
/// (`device:coleccion:seq`), así el mismo lote reenviado tras un timeout no duplica nada.
/// </summary>
public static class Lote
{
    ...
    private static void Coleccion(System.Text.StringBuilder sb, string nombre, IReadOnlyList<FilaDelSpool> filas)
    {
        ...
        for (int i = 0; i < filas.Count; i++)
        {
            ...
            // {"a":1} → {"seq":N,"a":1}   ·   {} → {"seq":N}
            sb.Append("{\"seq\":").Append(filas[i].Seq);
