...
/// <summary>
/// El secreto HMAC de la organización, cifrado en reposo con DPAPI (CurrentUser). Es la única
/// pieza con la que se podría re-derivar una huella, así que no puede quedar en claro en el disco
/// de un PC compartido. DPAPI lo ata al usuario de Windows de la máquina: copiar el archivo a otro
/// PC no sirve de nada.
///
/// Nunca se loguea, nunca entra en un lote, nunca sale de este proceso salvo hacia el HMAC en
/// memoria. Llega UNA vez en el enrolamiento (y en una rotación explícita).
/// </summary>
[SupportedOSPlatform("windows")]
internal static class Secreto
{
    private static readonly byte[] Entropia = "medidor-u-hmac-v1"u8.ToArray();
