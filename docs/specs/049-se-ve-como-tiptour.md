...
| 372 | (a) un ciclo de mentira con 7 candidatas (dos con la etiqueta «Buscar», ids `3) Buscar (Button)` y `6) Buscar (Button)`), decisión `Actuar` sobre `6) Buscar (Button)` con `Alternativas` de 7 probabilidades y `MsDecidir = 399`: `EstadoDeLaDecision.De(ciclo, coste)` devuelve `Cabecera.Paso == "paso 2"`, `Detectados == "· 7 detectados"`, `Ms == "399ms"`; `Barras.Count == 5`, ordenadas de mayor a menor, `Valor` con `"0.00"` invariante; las dos «Buscar» salen como `"3) Buscar"` y `"6) Buscar"` y las demás sin número; con `Pulsada == null` exactamente **una** `Resaltada`, la de id `6) Buscar (Button)`, marcada `PorElegida`; con `Pulsada = "6"` (el número que trae el paso del evento, `Paso.Numero`) la resaltada es la misma y `PorPulsada`; con `Pulsada = "3"` la resaltada pasa a `3) Buscar (Button)` —comparación por el **número del id**, `Ordinal`, por el mismo camino que `SurfaceMapTools.cs:318`— y el ticker es `"la 1.ª no estaba: pulsé la 2.ª «Buscar» (3)"`; y con 1.450 ms la cabecera dice `"1,450ms"`; **añadidas en la revisión del 2026-09-23, antes que su código** —el modelo se juzgaba con la pulsada puesta a mano y en la app nadie la ponía—: ~~`CicloDeJev.ConLaLinea(decidido, objetivo, línea)` con `"paso 2: «Buscar» (3) conf 0.40 · cambió"` da un ciclo `Linea` con `Pulsada == "3"` sacada de la línea~~ (**reescrito al juntar C y D, 2026-09-24: la prosa dejó de ser fuente**, §Revisiones): con un decidido cuya pulsada es «3» —la del paso—, `CicloDeJev.ConLaLinea(decidido, objetivo, línea)` con cada una de las cuatro palabras de B (353) —`"paso 2: «Buscar» (3) conf 0.40 · cambió de sitio"`, `· cambió dentro`, `· cambió delante`, `· no cambió`— da un ciclo `Linea` que **conserva** `Pulsada == "3"` y lleva la línea tal cual, y pintado resalta la `3) Buscar (Button)` `PorPulsada` con el ticker `"la 1.ª no estaba: pulsé la 2.ª «Buscar» (3)"`; sin pulsada en el paso, una línea que nombra un número **no la inventa**; con la línea de parada ninguna barra queda resaltada; y sin decisión el ciclo nace del objetivo, sin decisión ni pulsada inventadas. (b) `Ui/Jev/VistaDeJev.cs` pasa cada línea por `CicloDeJev.ConLaLinea(` en una línea de código | quitar el `OrderByDescending` de las barras, o resaltar por `Etiqueta ==` en vez de por el número del id |
...
| `CicloDeJev` | el registro de entrada: `Objetivo`, `Paso`, `Candidatas(Id, Etiqueta, Tipo, Caja?, EsLeida)`, `Decision?(Actuar, Puerta, Confianza, Alternativas, Cumplido?, Ausente?, Peligro, Porque)`, **`Pulsada?`** (número del id: `Paso.Numero` del evento desde el 2026-09-24; antes, de la línea de progreso) y **`CajaPulsada?`** (de la mano), `MsDecidir`, `TokensFacturados?`, `Fase`, `Linea` (progreso del tramo, tal cual); y desde la revisión de la fusión, **`NoSePulso`** + **`PorQueNoSePulso`** (el paso terminó y la mano no pulsó, y el porqué del paso: solo los pone el evento) y **`DeUnPaso`** (la línea viene de un decidido: la pone `ConLaLinea`) | datos | 372–375, 382 |
| `EstadoDeLaDecision` | `De(ciclo, coste) → LoQuePinta(Objetivo, Ticker, Punto, Resultados?(Cabecera, Medidores, Barras))`; la resaltada es la pulsada, la elegida solo mientras no se sepa qué se pulsó —con `NoSePulso` ya se sabe: ninguna— | sí | 372, 373, 374 |
...
| `CajasDelOverlay` | `De(candidatas, pulsadaId?) → cajas`, `ConPulsada(caja)`, `SinCaja`, `Caducar()`; `DelCiclo(ciclo)`: el decidido y la línea de un paso (`DeUnPaso`) pintan las cajas de su paso —vacías si no tenía—, lo demás no toca el overlay | sí | 375, 382 |
...
| `ObservadorDelDecisor` | `Oir(mapa, publicar)`: se suscribe una vez por mapa a `SurfaceMapTools.AlDecidir` (368) y publica `CicloDe(PasoDecidido)` —la decisión ya vetada, la pulsada del paso, las cajas solo si no cambió lo que se ve, y `NoSePulso` si el paso terminó sin pulsar—, que pasa por la traducción pura `CicloDe(objetivo, ofrecidas, decision, msDecidir, cajas)`. ~~`Envolver(interno, alDecidir)`, `Interno`, `EstaEnvuelto`~~: el puente provisional, retirado al juntar C y D (2026-09-24) | sí (la traducción) | 382 |
...
**2026-09-24, revisión de esa fusión** (siete lecturas independientes de `de9f1a0`; las siete encontraron el mismo
hueco). Oír el evento de C cambió **cuándo** llega el ciclo decidido —con el paso terminado, no al decidir— y dos
lecturas de la vista seguían suponiendo el momento del puente. Se amplían dos enunciados (374 y 382), literalmente
iguales aquí y en `Contrato.cs`; **ningún número nuevo, ninguno reciclado**.

| # | Hallazgo | Comprobación (M) | Qué cambió |
|---|---|---|---|
| 5 | **«Acciona» sobre lo que la mano no pulsó.** Con el puente, `Pulsada == null` con `Actuar` era «todavía no se sabe» y «Pulsando» con la elegida resaltada era verdad. Con el evento, ese mismo valor es «no se pulsó» (la elegida no estaba y no hubo segunda, homónimos, el tope), y el panel seguía diciendo «Pulsando «Crear Triage Administrativo» (2)» con su barra resaltada —con `map_decidir` fuera de tramo, lo último que se veía—. Dos causas en un valor (aprendizaje nº2); la misma clase que la vetada, en un segundo sitio (aprendizaje nº7). El juez de la 382 construía el caso y solo miraba `Pulsada == null` | rojo medido con el código de `de9f1a0`: el ciclo pintado dice «Pulsando «Crear Triage Administrativo» (2)» con el punto a 0,82, y con distribución deja 1 barra resaltada, también en su línea «· no pudo». Lectores de `Pulsada == null` como «no se sabe» en `Ui/Jev`: **2** (`EstadoDeLaDecision.Ticker` y `Resultados`); `CajasDelOverlay.De` no pinta rosa sin pulsada y no cambia | `CicloDeJev.NoSePulso`, que solo pone el evento; con él, «No pude pulsar «x» (n)» con 0,42 y ninguna barra resaltada. «No pude» y no «no estaba»: tres causas, y la línea del tramo dice cuál |
| 6 | **Sin decisión, el ticker en blanco.** Decisor apagado, pantalla sin nombre, nada accionable o el decisor lanzó: el ciclo salía sin decisión y sin porqué. En un tramo lo tapaba «sin acción · …»; con `map_decidir` el panel no decía por qué no hizo nada (patrón nº10) | rojo medido: con un decisor que lanza, el ticker es «» con 0,82 | `CicloDeJev.PorQueNoSePulso` = `Paso.Porque` tal cual; el ticker lo dice con 0,42 |
| 7 | **La línea de un paso sin nada no vaciaba el overlay.** `DelCiclo` reconocía la línea de un paso por su decisión o sus candidatas; un paso sin ninguna de las dos llega pegado a su línea, el conector pinta solo la línea, y el overlay se quedaba con las cajas del paso anterior —la caja que miente (patrón nº8)—. El comentario lo tenía como «límite dicho» | rojo medido: `DelCiclo(ConLaLinea(decidido sin nada, …))` da `null` | `CicloDeJev.DeUnPaso`, que pone `ConLaLinea` cuando hay un decidido detrás; `DelCiclo` pregunta eso |
...
pero antepone una causa que no es (aprendizaje nº2): Jev sí quería. `ElTramo.Paso` no distingue el veto de la duda;
llevar el `Veto` en el paso y decir «vetada: …» es un cambio de la 386 y va con su promesa, no en esta rama.
