...
> log…»). Después `7380fd8` gastó la **344** (tarde del 22), y `6554aa5` (Felipe, 12:45: el historial
> de GPT-Live, la 345) y `043addc` (Felipe, 13:00: el notch, 335–349) las **345–349**. `main` queda con
> **335–345 repetidas** —la 345, por ejemplo, es a la vez el chat abierto del notch y el historial de
> GPT-Live—: no es de esta spec y no se arregla aquí; se dice en el PR. Los nueve de esta
> spec son hoy **350 y 386–393**: la **350** sigue siendo la que era (nadie más la usa, **(M)**), la
> **388** la tomó al caer la 344 y las **389–393** son las que fueron 345–349 (ver Revisiones y
> Hallazgos). 351–385 son de las ramas B, C y D. Comprobado con `git grep` sobre las 49 refs
> `origin/*` y las ramas locales tras `fetch`, el 22-09 **(M)**: 389–399 libres. Nueve números, nueve
> promesas: lo que la arquitectura numeraba 350 (el clic físico que no cae en Ü) queda fuera, y se
> dice abajo por qué.
...
> `5574148` (342–343)—. Propuesta: ese documento pasa a **050** y sus promesas a **394+**; si el
...
que la de la 389, y por eso **queda roja hasta la fase 3** (se dice en cada fase). Su caso «cumplido
...
|---|---|---|---|
| 278, 279, 280 | `RespuestaChoice` (el ayudante común) añade la clave «0) ninguna» con 0 **y las dos nouls a 0,1** | desde la 391 esa clave viaja y la 388 exige que la respuesta traiga **exactamente** las que viajaron; desde la 389 una respuesta sin nouls no acciona, y 278 («buena», `:11642`) y 279 («justa», `:11661`) exigen `Actuar=true` | intacto |
...
| 275, 278–281, 289 | ninguno | sus seis `GetMethod("Elegir")` sin tipos siguen encontrando **un único** método: la larga se llama `ElegirConModelo` (392) | intacto |
...
| 289 | el fixture de hoy con `Respuesta()` completa (Σ = 1, con ninguna); el caso `viejo` (sin nouls) exige `!Actuar` y «falta» en el porqué | `Noul` vuelve a devolver 0 cuando la noul falta: el caso `viejo` acciona. Pone rojas **289 y 389 a la vez**, porque comparten la cláusula de ausencia; el veredicto tiene que nombrar las dos y ninguna más |

**Lo que los jueces ganaron en la fase 10** (revisión tras la fase 9, siete hallazgos; cada caso visto ROJO antes de su
línea, fila **Hecho** de la fase 10):

| # | Caso nuevo | Sabotaje de una línea |
|---|---|---|
| 388 | una clave **repetida** en `probabilities` —la elegida a 0,05, la otra a 0,90, y la otra otra vez a 0,0—: no acciona, y `QueNoCuadro` y `Porque` llevan «repetida «2) Buscar (Button)»» | `if (!vistas.Add(pr.Name)) { … continue; }` → `vistas.Add(pr.Name);` |
| 390 | «Send», «Delete», «Comprar ahora» y «Pagar» elegidas con 0,9 y **peligro 0,3** (bajo `PeligroMaximo`): 0 pulsos y «no se puede deshacer»; «Comprar ahora» como segunda mejor: no se pulsa | en `Vetada`, quitar `SafeToClick.EsDestructivo` de la condición |
| 390 | la línea «decisor:» de una elegida vetada **no dice ACCIONA** y sí «vetada» | en `ConVeto`, `new DecisionDeUnPaso(false, "", …)` → `(true, Puerta, …)` |
| 391 | `confidence` **0,91** y `probabilities[ninguna]` **0,80**: el porqué lleva «0.80» y no «0.91» | el porqué imprime `confianza` en vez de `pNinguna` |
| 393 | `uia://saplogon.exe/…` y `uia://sapgui.exe/x` sin la variable: 0 llamadas, no acciona, «U_DECISOR_SAP_TEXTO» y «Decide Luna», sin regla local; con `=si`, 1 llamada | `sapPorUia = … && SurfaceLocator.IsSap(…)` → `… && false` |
| 350 | parte 2 con la mano **realista** de `MapaParaDecidir` (cuenta «pulsé «{etiqueta}»»): el relato y **todas** las líneas del log de la llamada —«mapa-mcp ←» incluida— sin el texto; parte 3 con una mano que **no termina** («no pude pulsar «{etiqueta}»… Vivo aquí: «Triage/Urgencias»…»): el «la mano no pudo:» del tramo, al log y al notch, nombra «fila 2 (GuiGridFila)» y «fila 3 (GuiTreeFila)» y no lleva el texto de ninguna fila | `string cuenta = SinTextoDeFilas(Take(…));` → `string cuenta = Take(…);` |
...
```
FILTRAR  PoliticaDeLoQueViaja (393): sapgui:// sin habilitar u origin vetado ──▶ No(causa). Decide Luna. (0 llamadas)
...
VALIDAR  RespuestaDeJev.Validar(json, idsQueViajaron): la distribución ENTERA (388), TODAS las violaciones nombradas
   │     las nouls las juzga Noul → double? y el caso peor (389)
   │     fuera de forma ──▶ DecisionDeUnPaso.No con QueNoCuadro=«campo=valor · campo=valor», sin Alternativas
   │     en forma ──▶ compuertas, todas cerradas: ofrecida · ninguna (391) · cumplido≥0,70 (386: «Jev cree»)
   │                  · peligro≥0,50 · confianza<umbral
   ▼
PULSAR   UnPasoDecidido: candidatas = elegida + segunda, ambas por EsPeligrosa O EsDestructivo (390) ANTES del Take;
         la elegida vetada sale como decisión con Actuar=false y Veto, antes de registrarla (fase 10)
   ▼
...
| `cumplido.noul`, `peligro.noul` | presentes, número, finitas, en [0,1] | ≤ 1 + 1e-6 | `Noul` → `double?` (`null` = no vino, no es objeto o no es número) y el caso peor en `ConJev` (389). **No están en `Validar`**: si `Validar` las rechazara, `Noul` nunca correría con una ausente y el sabotaje de la 289 no podría aplicarse |
...
- `PeticionASystemOne.IdNinguna = "0) ninguna: nada de esta pantalla avanza hacia el objetivo"`. La añade
  ...
  de `UnPasoDecidido`. Si Jev la elige, `No("Jev eligió «ninguna» con probabilidad 0,80: no lo veo en esta pantalla —nada
  de lo que hay avanza hacia el objetivo—. No se acciona. Decide Luna.")` —**su probabilidad, `probabilities[«0) ninguna…»]`,
  no `confidence`**: hasta la fase 10 se imprimía la confianza con el nombre de la otra, y el juez ponía las dos a 0,80— —la frase «no lo veo en esta
  pantalla» es la que el juez de la 391 exige y la que D reutiliza—. **No es compuerta calibrada**: se
...
- `Elegir(quien, pantalla, objetivo, puertas, umbral, transporte)` **se conserva y sigue siendo el único
  ...
  instancia por reflexión) para que la 392 juzgue **el cableado real**, que es lo que estaba roto —
...
~~`SafeToClick.EsDestructivo` no sirve: excluye «guardar» a propósito.~~ **Corregido en la fase 10**: no
sirve *sola*, pero sin ella el veto no ve «Send», «Delete», «Comprar ahora» ni «Pagar» (**M**, rojo de la fase
10: con el código de `d905e6a` los cuatro se pulsaban; el hallazgo lo midió además por reflexión: `EsPeligrosa`
false y `EsDestructivo` true con los cuatro), y el veto ya frena al
decisor en `web://` y `uia://`. Son dos listas para la misma clase de error (patrón nº5): `Vetada` veta si
muerde **cualquiera** de las dos, con la misma frase («no se puede deshacer: te la dejo a ti.»).

Consecuencia deliberada: **el tramo y `map_decidir` nunca pulsan «Guardar» aunque el objetivo sea
guardar**. Lo irreversible lo pulsa la persona, o Luna con un `map_take` explícito, como hasta hoy. Y desde
la fase 10, tampoco lo que `EsDestructivo` lista para responder diálogos: «Aceptar», «Confirmar»,
«Permitir», «Instalar», «Compartir», «Subir», «Publicar», «Suscribir»… —con su negación respetada («No
eliminar» no la veta esa lista, pero «eliminar» la veta la otra)—. En un diálogo «¿Desea eliminar…?», «Aceptar»
**es** eliminar; que lo pulse la persona o Luna con `map_take` es fallar cerrado, y se dice en la cuenta.

**El veto es un dato de la decisión, no una nota detrás** (fase 10). `UnPasoDecidido` evalúa el veto de la
elegida **justo después de decidir y antes de registrar**, y la decisión vetada sale con
`DecisionDeUnPaso.ConVeto(veto, porqué)`: `Actuar=false`, `Puerta` vacía, `Veto` con el porqué del veto, y
la confianza, las alternativas, las nouls, lo que viajó y la señal intactos. La línea «decisor:» dice «no
acciona: vetada». **Contrato para las ramas C y D** (que al integrarse leen la decisión): «se pulsó» sale
del paso (`ElTramo.Paso.Actuo`), no de `DecisionDeUnPaso.Actuar`; y quien publique la decisión —el evento
`AlDecidir` de C— la publica **después** del veto, o publicará `Actuar=true` sobre algo que no se pulsó. D
pinta «vetada» cuando `Veto` no está vacío, igual que pinta `QueNoCuadro`.
...
| | |
|---|---|
| **Deja** | 350 y 386–393 escritas y `PENDIENTE`; la 289 reescrita con su fixture (Σ = 1, `viejo` invertido); `RespuestaChoice` con «0) ninguna» **y las dos nouls a 0,1**; el caso 1 de la 292 con `Cumplido=0,9` (su aserción nueva llega en la fase 8) |
...
| **Terminado** | `contrato-del-grafo.ps1` → **CONTRATO ROTO** exactamente por 350 y 386–393 y la 289; 343 y anteriores intactas (la 292 sigue verde: `Cumplido=0,9` elige la misma rama que hoy elige la subcadena) |
| **Hecho** | 2026-09-22. Veredicto literal: `CONTRATO ROTO: 23 promesa(s) incumplida(s). El cambio no puede entrar así.` Rojas **exactamente** 289, 388, 389, 390, 391, 392, 393, 350, 386, 387; **280 verdes**; 7 `PENDIENTE` (388, 389, 391, 392, 393, 350, 387) y 3 rojas por aserción contra el código de hoy (289: `viejo` acciona y no dice cuál falta; 390: pulsa «Grabar» elegida, pulsa «Guardar» como segunda, y el tramo pulsa «Grabar» tres veces; 386: la palabra «cumplido» del porqué elige la rama). El **23** es lo que el arnés cuenta desde siempre: aserciones fallidas, no promesas (16 aserciones + 7 pendientes) |
...
| **Terminado** | 389 y 289 verdes; 388 intacta; 278, 279 intactas (el ayudante ya manda nouls) |
| **Sitios** | 1 (`Noul`), 2 llamadores (cumplido, peligro) |
| **Hecho** | 2026-09-22. `Noul(answers, id, out crudo)` devuelve `double?` (`null` si no vino o no es número; `crudo` es el texto JSON tal cual: «80», «"sí"», «1e400», vacío si no vino) y `NoulCerrada(answers, id, peor, violaciones)` decide el caso peor y nombra la regla: `falta «peligro»` · `peligro=80 fuera de [0,1]` · `peligro="sí" no es número` · `peligro=1e400 no es finito`; ≤ 1+1e-6 se lee como 1 (la misma tolerancia que las probabilidades). Lo que no cuadra de las nouls se suma a `QueNoCuadro` (después de lo de `Validar`, con « · ») y `ConJev` no acciona por la misma salida que la 388: `Alternativas` vacía —con `Actuar=false` `map_decidir` no mira la segunda mejor, comprobado en `SurfaceMapTools.cs:296`—. **Sitios (M)**: `grep Noul(` en `windows-client/src` y `windows-graph/src` → 1 definición, 2 llamadores (`ElDecisor.cs:372-373`), 0 restos de «0 si no se preguntó». Antes de traer `main` el conflicto: `7380fd8` gastó la **344** y la de esta spec pasó a **388** (Hallazgos). Veredicto de partida tras el merge: `CONTRATO ROTO: 41` (rojas 289, 389, 390, 392, 393, 350, 386, 387; 388 verde). Veredicto literal al terminar: `CONTRATO ROTO: 17 promesa(s) incumplida(s). El cambio no puede entrar así.` — rojas exactamente 390, 392, 393, 350, 386, 387; **389 ✔, 289 ✔, 388 ✔, 278/279/280/282 ✔, 344 (main) ✔**. Voz: `VOZ ÍNTEGRA: el collar promete lo que dice prometer.` **Sabotajes (M), uno por promesa, con copia y `fc`**: (389) `ElDecisor.cs:488` `return peor;` → `return v;` en la regla de rango → `CONTRATO ROTO: 21`, 389 roja con «peligro 80: Peligro toma el caso peor, 1; salió 80» (y 1,2 · −0,01 · cumplido 1,5), la 289 verde como anticipaba la tabla de sabotajes; (289) `:484` `falta «{id}»` → `{id} no vino` → `CONTRATO ROTO: 20`, 289 roja por «y se dice cuál falta (dijo: «… cumplido no vino · peligro no vino»)» y 389 por «falta «peligro»» / «falta «cumplido»». Las dos veces: restaurada con `fc /B` sin diferencias, fecha del `.cs` tocada, `U.dll` posterior al `.cs` (11:22:05 < 11:22:10 y 11:27:56 < 11:28:01), y el juicio de vuelta a 17 |
...
| **Terminado** | 392 verde; 275–281, 289 y 290 intactas (`GetMethod("Elegir")` sigue encontrando uno solo) |
...
| **Hecho** | 2026-09-22. `ElegirConModelo(quien, pantalla, objetivo, puertas, umbral, transporte, modelo, politica)` en `ElDecisor.cs`; `Elegir` de seis delega con `ModeloPorDefecto` y `PoliticaDeLoQueViaja.PorDefecto` y sigue siendo **el único** método con ese nombre; `ConJev` recibe el modelo y lo pone en el cuerpo (vacío → alias por defecto, patrón nº9). `InterruptorDelDecisor` gana `FabricaDeTransporte` (propiedad; por defecto `ClienteTypeSafe.TransporteSegun`; el constructor de tres no cambia) y llama a `ElegirConModelo` con `cfg.Modelo`. **Lo mínimo que el código exigió fuera de la fase**: `PoliticaDeLoQueViaja` nace con SOLO `PorDefecto` (la 392 la pide por reflexión para delegar); `Leer`/`PuedeViajar`/`VetadosPorDefecto` siguen en la fase 6 y la 393 sigue `PENDIENTE` por ellas. **Sitios (M)**: `ElDecisor.Elegir(` en producción → 0 (el interruptor era el único, y pasa a `ElegirConModelo`); `ElegirConModelo(` → 1 delegación + 1 interruptor + 4 líneas de la sonda; `GetMethod("Elegir")` en el contrato → **18** literales sin tipos (no 6 como decía esta fase), sin tocar, y todos resuelven. La sonda no compila contra su `UBin` por defecto (`C:\U-dev2\bin`, un `U.dll` sin `Decision`); compilada con `-p:UBin=<Release del worktree>`: exit 0. Veredicto literal: `CONTRATO ROTO: 16 promesa(s) incumplida(s). El cambio no puede entrar así.` — rojas exactamente 390, 393, 350, 386, 387 (fases 5–8); **392 ✔, 275–282 ✔, 289 ✔, 290 ✔, 344 ✔, 389 ✔, 391 ✔, 388 ✔**. Voz: `VOZ ÍNTEGRA: el collar promete lo que dice prometer.` **Sabotaje (M)**: `InterruptorDelDecisor.cs:84` `cfg.Modelo` → `ConfiguracionDelDecisor.ModeloPorDefecto` (`fc /N` contra copia: una línea) → `CONTRATO ROTO: 17`, la 392 roja por «con U_TYPESAFE_MODELO=jev-1.13.0 el cuerpo lleva "model":"jev-1.13.0"; llevó «jev-latest»»; restaurada (hash SHA-256 idéntico a la copia), fecha tocada, `U.dll` del scratch posterior al `.cs` (11:40:05 < 11:40:23): 16 otra vez, 392 ✔. **Hallazgo del arnés (M)**: la primera corrida del contrato murió sin veredicto —`NO SE PUDO JUZGAR`— después de la 240 y sin línea de excepción, en medio de la 220 (la sesión de voz que abre un socket); la segunda corrida idéntica dio el 16. No se explica; se anota que el arnés puede morir mudo ahí y que el script lo distingue bien de un rojo |
...
| **Terminado** | 393 verde; 275, 277, 281 intactas |
...
| **Hecho** | 2026-09-22. `PoliticaDeLoQueViaja` gana `Leer(entorno)` (`U_DECISOR_SAP_TEXTO`: solo «si»/«sí», vacío = ausente; `U_DECISOR_TEXTO_VETADO`: por «;», sumado a los de por defecto), `VetadosPorDefecto` (`web://itsmiracleai.com`, `web://itsmiracleai.com.co`), `UbicacionQueViaja` (= `SurfacePlace.OriginOf`) y **un solo** `PuedeViajar(pantalla, out porque)`: `sapgui://` sin habilitar → «sapgui:// sin U_DECISOR_SAP_TEXTO=si: no se manda texto de SAP a Jev y no decido por regla local. Decide Luna.» (con un valor que no es «si», lo dice: «con U_DECISOR_SAP_TEXTO=«no» (se habilita solo con «si»)»); origin vetado → ««web://app.itsmiracleai.com» está vetado para Jev (cae bajo «web://itsmiracleai.com»)…». Cómo veta: en `web://`, el host y sus subdominios, sin puerto; en otro esquema, el prefijo de la pantalla; una entrada **sin esquema** («historia»), el host en cualquier esquema —decisión de esta fase, no de la tabla de diseño: si no, quedaría sin vetar nada en silencio (aprendizaje nº18)—. `ConfiguracionDelDecisor.Leer` la lee en `Politica`; `ElDecisor.ConJev` la consulta **antes** del transporte (`No(porque)`, sin `Simulado`; `politica == null` → `PorDefecto`) y pone en el `state` solo `UbicacionQueViaja(pantalla)`. **Lo mínimo que el código exigió fuera de la lista**: `InterruptorDelDecisor.cs:84` pasa `cfg.Politica` en vez de `PorDefecto` —sin esa línea las dos variables se leían y no hacían nada en producción—, y para que esa línea tuviera juez **se añadió al juez de la 393 su parte 4, el cableado por el interruptor** (ver Revisiones), vista **roja antes de la línea** (`CONTRATO ROTO: 9`, la 393 roja solo por «U_DECISOR_TEXTO_VETADO=web://historia veta web://historia/x… (se llamó 1)» y «U_DECISOR_SAP_TEXTO=si habilita sapgui://: 1 llamada (se llamó 0)»). Además, solo comentario y doc: la prosa falsa de `PeticionASystemOne.EstadoDeLaPantalla` y `docs/el-decisor-y-typesafe.md` §*Lo que se le manda* (ahora una tabla que dice que las etiquetas de `uia://`/`web://` **sí salen**, que el objetivo va tal cual, y que las filas siguen viajando con texto hasta la fase 7), más las dos variables en su tabla. **Sitios (M)**: `StartsWith("sapgui://", StringComparison.OrdinalIgnoreCase)` en `windows-client/src` + `windows-graph/src` → **17** = los 16 de antes sin tocar (`RellenadorSap.cs:49`, `SurfaceMapTools.cs:1234, :2703`, `LoQueHaceLaSkill.cs:77`, `MundoQueToca.cs:39, :101, :134, :179`, `TerrenoParaElVisor.cs:74`, `TerrenoPorDelante.cs:137`, `Leccion.cs:290`, `FaceWindow.xaml.cs:284, :952, :5497, :5525`, `SurfacePlace.cs:97`) + la política (`PoliticaDeLoQueViaja.cs:94`); fuera de esas dos carpetas hay 1 más, `mapeador/Pulso/ComoMePongoDelante.cs:58`. `PuedeViajar(` → 1 definición, 1 llamador (`ConJev`). `EstadoDeLaPantalla(` en producción → 1 llamador, y recibe el origin; recorte de la ubicación → 1 (`UbicacionQueViaja`, por `OriginOf`), usado por el `state` y por `PuedeViajar`. Veredicto de partida (tras `3cbed31`): `CONTRATO ROTO: 8`, 303 verdes. Veredicto literal al terminar: `CONTRATO ROTO: 7 promesa(s) incumplida(s). El cambio no puede entrar así.` — rojas exactamente 350, 386, 387 (fases 7–9); **304 verdes**; 393 ✔, 275–282 ✔, 289 ✔, 290 ✔, 388–392 ✔, 126 ✔, 344 ✔. Voz: `VOZ ÍNTEGRA: el collar promete lo que dice prometer.` **Sabotaje (M)**: el de la tabla, `PuedeViajar` devuelve siempre `true` —`PoliticaDeLoQueViaja.cs:91` `porque = "";` → `porque = ""; return true;` (`fc /N` contra copia: una línea)— → `CONTRATO ROTO: 17`, la 393 roja por 10 aserciones (en `sapgui://` «se llamó 1» y «salió Actuar=True», el porqué sin «Decide Luna», vacía y «si» contando 2 y 3 llamadas, `web://app.itsmiracleai.com` «llamadas=1, Actuar=True» y su porqué sin el origin vetado, el de la variable «llamadas=2», y por el interruptor «se llamó 1» dos veces) y **ninguna otra promesa nueva**; el compilador avisó `CS0162` en `PoliticaDeLoQueViaja.cs(92,9)`, que prueba que el binario juzgado era el saboteado (`.cs` 19:20:03 < `U.dll` 19:20:22). Restaurada desde la copia (`fc /B` sin diferencias, SHA-256 idéntico `4DFAA892…`), fecha del `.cs` tocada, `U.dll` posterior (19:21:52 < 19:22:01): 7 otra vez, 393 ✔ |
...
| **Hecho** | 2026-09-22. `PoliticaDeLoQueViaja` gana `TiposDeFila` (los tres, exactos), **un solo** reconocedor `EsFila(id, out numero, out tipo)` —el inverso de «N) etiqueta (Tipo)»: el tipo es el último paréntesis—, `IdQueViaja` («2) fila (GuiGridFila)»; sin número, «fila (GuiGridFila)»; el resto tal cual) y `NombreParaContar` («fila 2 (GuiGridFila)», o la etiqueta). `ElDecisor.ConJev` arma a la vez la lista que viaja —en el choice **y en el state**— y el mapa id que viaja → id ofrecido; la respuesta se valida contra lo que viajó (388) y vuelve a la ofrecida: `Puerta` y `Alternativas` (así la segunda mejor de la 288 sigue encontrando su selector), mientras el `Porque` nombra lo que viajó. Toda decisión que sale después de entregar el cuerpo lleva `Viajaron`, `FilasSinTexto` y `Caracteres` (la cola de `ConJev` pasa a un `Juzgar` con el mismo cuerpo, salvo el mapa; `Con` conserva las tres). `UnPasoDecidido` nombra cada puerta con `NombreParaContar`: la línea `decisor:` («viajan 4 de 4 · 3 filas sin texto · 1234 caracteres», «ACCIONA «fila 2 (GuiGridFila)»»), el relato, el veto y el `Paso` del tramo. **Lo mínimo que el código exigió fuera de la lista, con su juez** (Revisiones): (1) el mapa necesita ids de viaje únicos y dos filas sin número chocarían: `ConJev` no pregunta y lo dice; (2) el veto de la fase 5 escribía la etiqueta en dos salidas más, y `PorQue` nombra por la etiqueta: `Vetada` sustituye el nombre dentro de su frase. Las dos partes nuevas del juez se vieron **rojas antes de sus líneas** (`CONTRATO ROTO: 11`, la 350 roja solo por sus 5 aserciones nuevas; partes 1 a 3 ya verdes). `ElTramo.cs` no se toca. Doc: la fila de las filas en `docs/el-decisor-y-typesafe.md`. **Sitios (M, `grep` en `windows-client/src` y `windows-graph/src`)**: construyen los tres tipos **5** sitios, no 3 —`MundoQueToca.cs:238, :335, :351`, `FaceWindow.xaml.cs:380, :5399`—, los 5 con el literal exacto; más 3 consumidores exactos de la demo y 1 rama inerte (`MundoQueToca.cs:360-362`, Hallazgos). Salidas con la etiqueta en `UnPasoDecidido`: **5**, no 3 (las 3 de la spec + la cuenta del veto de la elegida con su «✋» + la «✋» de la segunda), alimentadas desde **1** sitio (`selectorDe`, campo `Nombre`); `.Etiqueta` queda en `UnPasoDecidido` solo para armar el id y para `EsPeligrosa`/`PorQue`. `EsFila` → 1 definición, 2 llamadores (`IdQueViaja`, `NombreParaContar`); `IdQueViaja(` → 1 llamador (`ConJev`); `NombreParaContar(` → 2 (`selectorDe` y `Nombre`). Veredicto de partida (tras `2d3b0c1`): `CONTRATO ROTO: 7`, 304 verdes. Veredicto literal al terminar: `CONTRATO ROTO: 6 promesa(s) incumplida(s). El cambio no puede entrar así.` — rojas exactamente 386 y 387 (fases 8–9); **305 verdes**; 350 ✔, 282 ✔, 285–295 ✔, 289 ✔, 388–393 ✔. Voz: `VOZ ÍNTEGRA: el collar promete lo que dice prometer.` (la primera corrida final de la voz murió **en `dotnet build`**, «Fatal error. Internal CLR error. (0x80131506)», sin veredicto; la segunda, idéntica, dio ese: es el arnés, no la voz). **Sabotaje (M)**: el de la tabla, `IdQueViaja` devuelve el id tal cual —`PoliticaDeLoQueViaja.cs:187` `return numero.Length > 0 ? $"{numero}) fila ({tipo})" : $"fila ({tipo})";` → `return id;` (`fc /N` contra copia: una línea)— → `CONTRATO ROTO: 12`, la 350 roja por 6 aserciones (el cuerpo lleva el texto de las filas; no lleva «2) fila (GuiGridFila)»; la respuesta no vuelve a la ofrecida —no cuadra: sobran y faltan claves—; «salió 4 · 0 · 1491»; y las dos de la parte 1b) y **ninguna otra promesa**; `.cs` 19:42:00 < `U.dll` 19:42:30. Restaurada desde la copia (`fc /B` sin diferencias, SHA-256 idéntico `87DF8A9B…`), fecha del `.cs` tocada, `U.dll` posterior (19:43:55 < 19:44:15): 6 otra vez, 350 ✔ |
...
| **Hecho** | 2026-09-22. Punto de partida: el parche del intento que se colgó (`parche-A-intento-colgado.patch` = `A-fase8-wip-1b5eaf8.patch`, SHA-256 `E44CB222…3179D`), que `git apply --check` aplicó limpio sobre `1b5eaf8` y que, revisado con el diff, toca exactamente los sitios de esta tabla. `SurfaceMapTools.cs:311`: `Sin(…, cumplido: d.Cumplido >= ElDecisor.CumplidoMinimo)` —se quita `\|\| d.Porque.Contains("cumplido")`—: lo decide el número, no la prosa del porqué. `ElTramo.cs:161-163`: el motivo pasa de «el objetivo ya está cumplido: {porqué}» a «Jev cree que ya está: {porqué}»; la cuenta sigue devolviendo el inventario detrás. `ElDecisor.cs:521-524`: «Jev cree que el objetivo ya está cumplido en esta pantalla (0.90): no acciono más. Que lo esté lo dice la llegada, no el modelo. Decide Luna.» (conserva «ya está», que es lo que juzga la 289). `Contrato.cs:12550`, **en el mismo commit**: el caso 1 de la 292 pasa de `Contains("cumplido")` a `Contains("Jev cree que ya está")` —juzga la rama, no la palabra del fixture—. **Lo mínimo fuera de la lista**: ningún código; solo la prosa de `docs/el-decisor-y-typesafe.md` (§ las tres preguntas y § `map_tramo`), que decía «se dice que ya está». **Decisión de esta fase** sobre el hallazgo de la fase 5 (el prefijo «el decisor no se atrevió:» reúne 7 causas, entre ellas el veto): se queda como está, porque la 386 exige ese literal para el caso `Cumplido=0,1` y cambiarlo sería cambiar el juez, no la rama; va a una rama propia. **Sitios (M, `grep` en `windows-client/src` y `windows-graph/src`)**: `Contains("cumplido"` → **1** antes (`SurfaceMapTools.cs:308` en `1b5eaf8`), **0** después; construcciones de `Paso.Cumplido=true` → 1 (`:311`), por número; `CumplidoMinimo` → 1 definición (`ElDecisor.cs:280`) y 2 lectores (`ElDecisor.cs:521`, `SurfaceMapTools.cs:311`); motivo del tramo → 1; mensaje del decisor → 1; aserción ajena → 1; «el objetivo ya está cumplido:» como texto que sale → 0 (queda en 2 comentarios que cuentan la historia, `ElDecisor.cs:520` y `ElTramo.cs:157`). Veredicto de partida (tras `1b5eaf8`, árbol limpio, 21:58): `CONTRATO ROTO: 6`, 305 verdes, rojas la 386 (5 aserciones) y la 387 (`PENDIENTE`). Veredicto literal al terminar: `CONTRATO ROTO: 1 promesa(s) incumplida(s). El cambio no puede entrar así.` — roja exactamente la 387 (fase 9, `PENDIENTE`); **306 verdes**; 386 ✔, 289 ✔, 292 ✔, 350 ✔, 388–393 ✔. Voz: `VOZ ÍNTEGRA: el collar promete lo que dice prometer.` (46 verdes). **Sabotaje (M)**: el de la tabla, volver a `\|\| d.Porque.Contains("cumplido", StringComparison.OrdinalIgnoreCase)` en `SurfaceMapTools.cs:311` (`git diff --no-index` contra copia: 1 línea, `@@ -311 +311 @@`, CRLF conservado: 2988 de 2988) → `dotnet build` 0 errores → `CONTRATO ROTO: 3`, la 386 roja por sus 2 aserciones del caso `Cumplido=0,1` («paré: Jev cree que ya está: Jev eligió «1) A (Button)»…» en vez de «no se atrevió», y «declara» algo sobre el objetivo), la 387 igual que antes (`PENDIENTE`) y **ninguna otra promesa** (la 292 y la 289 siguen verdes: el caso `Cumplido=0,9` elige la misma rama con sabotaje o sin él); `.cs` 22:03:53 < `U.dll` 22:04:35. Restaurada desde la copia (`fc /B` sin diferencias, SHA-256 idéntico `6F126A5D…`), fecha del `.cs` tocada, `U.dll` posterior (22:05:59 < 22:06:39): 1 otra vez, 386 ✔ |
...
| **Terminado** | 387 verde; 350, 386 y 388–393 intactas; `contrato-del-grafo.ps1` → **CONTRATO INTACTO** |
| **Sitios** | 1 lectura de `usage` (hoy 0); 1 composición para 2 salidas |
| **Hecho** | 2026-09-22. **El juez primero** (Revisiones): el enunciado ya decía «los tres salen en la cuenta y en la línea», y el juez solo miraba la línea; gana tres aserciones —la cuenta de `map_decidir` lleva, de «masa5» a «tokens 312», **exactamente** lo mismo que la línea (una composición, no dos); sin accionar, línea y cuenta igual y 0 pulsos; sin `usage`, la cuenta también dice «tokens sin medir»—. **Visto rojo antes de sus líneas (M)**: con solo las tres propiedades declaradas (`N`, `Masa5 = NaN`, `InputTokens`), `CONTRATO ROTO: 11 promesa(s) incumplida(s).` — roja **solo** la 387, 306 verdes, por 11 aserciones: N 0 ≠ 6, masa `NaN` ≠ 0,98 y tokens `null` ≠ 312 (accionando y sin accionar), la línea sin «masa5» ni «tokens sin medir», y las tres nuevas. **Lo hecho**: `DecisionDeUnPaso` gana `N`, `Masa5` (`NaN` = sin medir: no se preguntó, no contestó o la distribución no cuadró —la masa de una inválida no es una masa, como su segunda mejor—) e `InputTokens` (`null` = sin medir), y **`Senal()`**, la única composición: «N=6 · masa5 0.98 · 1.2× lo plano · tokens 312» («lo plano» = k/N, k = min(5, N)). `ElDecisor.ConJev`: N = las claves del choice sin blancos ni repetidas, por el mismo camino que `CuerpoDeEleccion`; `Juzgar` lee `usage` justo después de parsear —antes de cualquier compuerta, así va también en las que no accionan— y la masa de las alternativas validadas; `Contada` pone la señal en **cada** decisión que sale después de entregar el cuerpo (transporte que lanza: N sí, masa y tokens sin medir). `SurfaceMapTools.UnPasoDecidido`: `d.Senal()` una vez, en la línea `decisor:` (tras «caracteres», así la 350 sigue leyendo «viajan 4 de 4 · 3 filas sin texto · N caracteres») y en las 4 ramas de la cuenta que salen tras la decisión —no acciona, elegida vetada, no ofrecida, y el relato pegado a la primera candidata—. **Lo mínimo fuera de la lista**: `Con` y `ConLoQueViajo` copian las tres propiedades (sin eso, la señal volvía a «sin medir» en silencio) y `ConLaSenal` es la tercera copia; y la prosa de `docs/el-decisor-y-typesafe.md`. **Sitios (M, `git grep` en `windows-client/src` y `windows-graph/src`)**: lecturas de `usage` de la respuesta de Jev → **0** antes (la única coincidencia en `HEAD` es `/agent/usage` de `FaceWindow.xaml.cs:1014`, otra cosa), **1** después (`ElDecisor.cs:612-616`, llamada en `:505`); composición → **1** (`ElDecisor.cs:125`), llamada 1 vez (`SurfaceMapTools.cs:304`) para **2 salidas**, la línea (`:306`) y la cuenta (`:314`, `:346`, `:378`, `:395-405`); copias de `DecisionDeUnPaso` → 3, las 3 con las tres propiedades; **compuertas que leen `N`, `Masa5` o `InputTokens` → 0** (solo `Senal()` los lee). Veredicto literal al terminar: `CONTRATO INTACTO: el grafo se comporta como el día que se congeló.` — **307 verdes**; 387 ✔, 386 ✔, 350 ✔, 388–393 ✔, 289 ✔, 292 ✔. Voz: `VOZ ÍNTEGRA: el collar promete lo que dice prometer.` (46 verdes). **Sabotaje (M)**: el de la tabla, `masa5 = Masa5De(leida.Alternativas);` → `masa5 = 0;` en `ElDecisor.cs:522` (`git diff --no-index` contra copia: 1 línea, `@@ -519,7 +519,7 @@`, CRLF conservado: 729 de 729) → `dotnet build` 0 errores (`.cs` 22:19:44 < `U.dll` 22:20:02) → `CONTRATO ROTO: 2 promesa(s) incumplida(s).`, roja **solo** la 387 por sus 2 aserciones de la masa («accionando»/«sin accionar»: «es 0,98; salió 0»), 306 verdes. Restaurada desde la copia (`fc /B`: «no se han encontrado diferencias»; SHA-256 idéntico `269E6B02…75DB`), fecha del `.cs` tocada, `U.dll` posterior (22:21:56 < 22:22:06): `CONTRATO INTACTO`, 307 verdes |

### Fase 10 — revisión tras la fase 9: siete hallazgos, seis ciertos (dos eran el mismo)

| | |
|---|---|
| **Promesas** | 350, 388, 390, 391, 393 (los jueces se endurecen; los enunciados de **390** y **393** se amplían para decir lo que ahora se juzga) |
| **Toca** | `Decision/ElDecisor.cs` (`Validar`: clave repetida; «ninguna» con su probabilidad; `DecisionDeUnPaso.Veto` y `ConVeto`), `Decision/PoliticaDeLoQueViaja.cs` (SAP visto por UIA), `Uia/SurfaceLocator.cs` (`IsSap` de `private` a `internal`, nada más), `Mcp/SurfaceMapTools.cs` (`UnPasoDecidido`: `SinTextoDeFilas`, `Vetada` con las dos listas, el veto antes de registrar), `docs/el-decisor-y-typesafe.md` |
| **Terminado** | las cinco verdes con sus casos nuevos; 285–295, 386, 387, 389, 392 intactas; **CONTRATO INTACTO** |
| **Hecho** | 2026-09-22. **Los hallazgos, verificados uno a uno**: (1) **cierto** — el relato de `map_decidir` pegaba la cuenta de la mano, que la mano real redacta por la etiqueta (`PulsarSegunElNucleo.cs:241`, `RecorrerSegunElNucleo.cs:253`), y en una `GuiGridFila` la etiqueta es el texto de la fila (`MundoQueToca.cs:334-335`); el juez de la 350 no lo veía porque sus partes 2 y 3 usaban una mano falsa que no repetía la etiqueta. Arreglo: `SinTextoDeFilas` sustituye, entre «», la etiqueta y el selector de **cada** fila de la pantalla por su nombre para contar —todas viajaron sin texto— en la cuenta de la mano (**1** sitio: el `Take` de `UnPasoDecidido`, que alimenta las **3** ramas del relato y, por el `Paso`, el «la mano no pudo:» del tramo) y en la frase del veto (`Vetada` deja su `Replace` propio: un mecanismo para los dos). (2) y (6) **ciertos, el mismo**: con una clave repetida `vistas` (HashSet) y `leidas` (lista) discrepaban y la suma y el máximo se apagaban sin violación; ahora «repetida «k»» es una violación con nombre, la repetida no se lee, y la suma y el máximo se juzgan contra las **propiedades** que vinieron (con una repetida, no se juzgan sobre datos ambiguos). (3) **cierto** — «ninguna» imprimía `confidence`; ahora `probabilities[ninguna]` (la distribución ya está validada, así que está). (4) **cierto en su vía medida** — `uia://saplogon.exe/…` viajaba; ahora `PuedeViajar` lo trata como SAP con **el criterio del localizador que lo acuña** (`SurfaceLocator.IsSap`, el proceso empieza por «sap»), no con `Mundos.EsSapVistoPorUia`, que solo conoce `saplogon.exe`. Su segunda vía (`CamposDeSap` relee `DondeEstoy()`) y la del portal visto por UIA quedan **sin tocar**: son deducidas y no tienen juez posible sin leer UIA real (Hallazgos). (5) **cierto** — `Vetada` consulta ahora `PuertasPeligrosas.EsPeligrosa` **o** `SafeToClick.EsDestructivo`. (7) **cierto** — el veto pasa a ser un dato de la decisión (`ConVeto`: `Actuar=false`, `Veto`), evaluado antes de la línea «decisor:», que dice «no acciona: vetada»; y el contrato para C y D queda escrito en *El veto determinista*. **Rojo antes del código (M)**: con solo los jueces nuevos, `CONTRATO ROTO: 26 promesa(s) incumplida(s). El cambio no puede entrar así.` — rojas **exactamente** 388 (4 aserciones: la repetida acciona), 390 (11: «Send», «Delete», «Comprar ahora», «Pagar» pulsadas; «Comprar ahora» pulsada como segunda; la línea «decisor:» con «ACCIONA»), 391 (2: «(0.91)»), 393 (5: `uia://saplogon.exe` y `uia://sapgui.exe` con «llamadas=1», «=2», y «se llamó 3» con la variable) y 350 (4: el relato con «pulsé «fila de prueba…»», «mapa-mcp ←» con el texto, el «la mano no pudo:» del tramo con el texto en el log y en el notch); **302 verdes**. **Verde (M)**: `CONTRATO INTACTO: el grafo se comporta como el día que se congeló.` — **307 verdes**. Voz: `VOZ ÍNTEGRA: el collar promete lo que dice prometer.` (46 verdes). **Sabotajes (M)**, uno por promesa y dos para la 390 (ganó dos cláusulas), en cadena —cada corrida enseña la promesa saboteada roja **y la anterior otra vez verde**, que es la prueba de que su restauración entró en el binario; la última restauración la prueba el INTACTO final—; cada uno con copia, cambio de una línea conservando bytes (`git diff --no-index`: `1 insertion(+), 1 deletion(-)`, sin BOM, CRLF intacto), `.cs` anterior al `U.dll` juzgado, y restauración con SHA-256 idéntico y fecha tocada: (388) `ElDecisor.cs:257` → `vistas.Add(pr.Name);` → `CONTRATO ROTO: 2`, roja solo la 388 por «repetida «2) Buscar (Button)»» (`.cs` 22:55:55 < `U.dll` 22:56:04); (391) `:607` `pNinguna` → `confianza` → `CONTRATO ROTO: 2`, roja solo la 391, 388 verde otra vez (22:57:25 < 22:57:35); (390, veto como dato) `:134` `ConVeto` con `(true, Puerta, …)` → `CONTRATO ROTO: 1`, roja solo la 390 por «la línea «decisor:» no dice ACCIONA», 391 verde (22:58:55 < 22:59:05); (390, lo destructivo) `SurfaceMapTools.cs:336` → `if (!peligrosa) return "";` → `CONTRATO ROTO: 10`, roja solo la 390 por sus 10 aserciones destructivas, la de «ACCIONA» verde otra vez (23:00:25 < 23:00:35); (350) `:419` sin `SinTextoDeFilas` → `CONTRATO ROTO: 4`, roja solo la 350 por sus 4 aserciones nuevas —la parte 4, el veto de una fila, **sigue verde**: su frase pasa por el mismo `SinTextoDeFilas` desde `Vetada`— y 390 verde (23:01:58 < 23:02:08); (393) `PoliticaDeLoQueViaja.cs:103` → `&& false;` → `CONTRATO ROTO: 5`, roja solo la 393, 350 verde (23:03:29 < 23:03:39). Restaurado el último (SHA-256 `FF96C0C0…` idéntico, 23:05:00 < `U.dll` 23:05:10): `CONTRATO INTACTO`, 307 verdes. **Sitios (M, `git grep` en `windows-client/src` y `windows-graph/src`)**: la cuenta de la mano entra en `UnPasoDecidido` por **1** `Take(` y sale por **3** ramas del relato + el `Paso` del tramo; las listas de lo irreversible son **2** (`PuertasPeligrosas.Verbos`, `SafeToClick.Destructivo`) más la del explorador autónomo (`SafeToClick.Prohibido`, otra pregunta), y quien veta por etiqueta en un camino de pulsar son `Vetada` (ahora las dos), `InstanciarSkill.cs:46` (**solo** `EsPeligrosa`: Hallazgos), `SurfaceMapTools.cs:2421` y `PulsarSegunElNucleo.cs:217` (solo `EsDestructivo`, a propósito: responder diálogos y repetir un clic); `EsDestructivo(` pasa de 2 llamadores a 3. El criterio «el proceso empieza por «sap»» está copiado en **4** sitios (`SurfaceLocator.cs:281`, `UiInspector.cs:147`, `AgentLoop.cs:232`, `SurfaceDetector.cs:37`); la política **no** es el quinto: llama al del localizador. `vistas.Add(` en `Validar` → 1. Copias de `DecisionDeUnPaso` → **4** (`Con`, `ConLoQueViajo`, `ConLaSenal`, `ConVeto`), las 4 con `Veto`; «ACCIONA» se escribe en 1 sitio, y ahora mira la decisión ya vetada |
...
| Refactorizar las 16 comparaciones de `sapgui://` | se nombran, no se tocan: la 393 compara por el mismo camino |
...
- **2026-09-22.** `RespuestaChoice` (`Contrato.cs:11532`) es el ayudante de 275–281: con la 388 y la
  391 tiene que mandar la clave «ninguna», y con la 389 **las dos nouls**, o 278 y 279 caen (afirman
...
- **2026-09-22, fase 0.** `Utf8JsonWriter` escapa lo no ASCII por defecto («·» → `·`, «í» →
  ...
  el bug no es un criterio). Los jueces de la 393 y la 350 miran el cuerpo **decodificado**
...
- **2026-09-22, fase 0.** El primer juez de la 388 listaba 1,0000001 entre lo que no cuadra; la tabla de
  ...
  la 389: se lee como 1, que cierra la compuerta.
...
- **2026-09-22, tras la fase 4.** Cuarta vez en el día: al traer `main` (`6554aa5` y `043addc`, Felipe)
  el conflicto fue otra vez literal —un `Prueba("345.` de GPT-Live contra el bloque de esta spec—, y
  `main` ya usaba además 346–349 para el notch. Como `main` no se toca, las cinco de esta spec pasan a
  **389–393** (345→389, 346→390, 347→391, 348→392, 349→393; libres en las 49 refs `origin/*` y en las
  ramas locales, **(M)**). La 350, la 386–388 y la 289 no cambian. Los commits `f1d0743`, `07f859b`,
  `3317a06`, `f3da616` y `bfe257b` las citan con el número viejo: lo que dicen de la 345…349 es lo que
  hoy se llama 389…393. Y `main` queda con **335–345 repetidas** (dos specs ajenas chocaron entre sí):
  el arnés las corre igual —cada `Prueba` es una llamada, no una clave—, pero un veredicto que diga
...
- **2026-09-22, fase 6.** La tabla del juez de la 393 no cubría el cableado del interruptor: con `PorDefecto`
  en `InterruptorDelDecisor.cs:84` la promesa salía verde y las dos variables no hacían nada en producción
  —`U_DECISOR_TEXTO_VETADO` leída y sin vetar: un guardia que se cree puesto (aprendizaje nº18)—. Se añadió la
  parte 4 del juez antes de la línea, y se vio roja por sus dos aserciones **(M)**. Ver Revisiones.
- **2026-09-22, fase 6.** Dos decisiones de esta fase que la tabla de diseño no fijaba, y se dicen: una entrada de
  `U_DECISOR_TEXTO_VETADO` **sin esquema** («historia») veta ese host en cualquier esquema, en vez de no casar con
  nada en silencio; y `U_DECISOR_SAP_TEXTO` acepta «si» y «sí» (cualquier otro valor no habilita y el porqué dice
  qué se leyó). Las dos fallan hacia cerrado. Ninguna tiene aserción propia en el juez **(D)**.
- **2026-09-22, fase 6.** SAP visto **por UIA** (cuando el Scripting no contesta, la ubicación cae a `uia://…`)
  no lo cubre la regla de `sapgui://`: su título ya no viaja (solo el origin), pero lo que UIA publique como
  etiqueta sí. `CLAUDE.md` dice que dentro de SAP GUI UIA ve un `Pane` opaco, así que el riesgo es bajo **(D, sin
  medir)**; si el dueño lo quiere cerrado, es un veto por defecto más (`uia://<proceso de SAP>`), con su número.
- **2026-09-22, fase 6.** `sondas/DelDecisor/Programa.cs:61-64` imprime un cuerpo armado a mano con la
  `Pantalla` entera y sin pasar por la política; no lo manda —la llamada real (`:84`) va por `ElegirConModelo`
  con `PorDefecto`—, pero lo que enseña ya no es lo que produce `ElDecisor`. Fuera de la fase, como su hallazgo
  de la fase 5.
- **2026-09-22, fase 6.** El interruptor no dice qué política está en vigor: `Estado` nombra quién decide, el
  modelo y el umbral, no si SAP está habilitado ni qué orígenes están vetados. Para el nivel 4 (punto 3) la
  prueba de que «decide Luna» en SAP es el porqué de cada decisión en la línea `decisor:`, no el botón.
  Hallazgo para la rama D (el panel).
- **2026-09-22, fase 7.** Las salidas que escribían la etiqueta de la puerta elegida eran **5, no 3** **(M)**: a
  las tres de la spec (`decisor:`, el relato, `paso k:`) la fase 5 sumó la cuenta del veto de la elegida con su
  «✋ … vetada la elegida» y el «✋ … vetada la segunda» con su cola en el relato; y `PorQue` nombra por la
  etiqueta. Es el aprendizaje nº11 otra vez —cablear dos de tres—, y por eso el juez ganó su parte 4. Las cinco
  salen de **1 sitio**, la tupla de `selectorDe` en `UnPasoDecidido`, que ahora lleva el nombre para contar.
- **2026-09-22, fase 7.** Los sitios que **construyen** los tres tipos de fila son **5, no 3** **(M, `grep`)**:
  `MundoQueToca.cs:238` (la atribución del clic en un árbol), `:335`, `:351`, `FaceWindow.xaml.cs:380` (la 379 de
  la spec) y `:5399` (las cajas del recuerdo, que no son puertas del decisor). Los cinco con el literal exacto, así
  que se reconocen los cinco; más 3 consumidores de la demo (`FaceWindow.xaml.cs:5228, :5230, :5235`), también
  exactos. Y una **sexta rama inerte**: `MundoQueToca.cs:360-362` emitiría una fila de árbol con el tipo de su
  shell, que `EsFila` no reconocería y viajaría con texto; hoy no corre —el único `new SapVisualElement(`
  (`SapGuiSurface.cs:1500`) pasa `IsNode: false`—. Queda dicho en `TiposDeFila`; si se despierta, su tipo tiene
  que ser uno de los tres.
- **2026-09-22, fase 7.** El mapa id que viaja → id ofrecido necesita que el primero sea único, y la política no lo
  garantiza sola: dos filas **sin número** viajarían las dos como «fila (GuiGridFila)». Con las puertas numeradas
  de `UnPasoDecidido` no pasa; con una lista sin numerar que lleve filas sí (hoy solo la del juez: la sonda
  `sondas/DelDecisor` no numera, pero tampoco lleva filas, **(M)** `Programa.cs:22-23`). Quedarse con una sería
  adivinar: `ConJev` no pregunta y lo dice. La misma puerta repetida no es ambigua y sigue como estaba (la
  deduplica `CuerpoDeEleccion`).
- **2026-09-22, fase 7.** Quedan salidas con el texto de una fila **fuera de la 350**, anteriores a Jev y sin tocar
  **(D, lectura)**: las `mano:` de `map_take` (ya dicho arriba); `mapa-mcp →` (`SurfaceMapTools.cs:2127`), que
  registra los argumentos de cada herramienta —`map_take exit=<etiqueta>` cuando Luna pide una fila por su nombre,
  que es lo que la promesa 183 le deja hacer—; y `mapa-mcp ←` (`:2216`), que registra los primeros 200 caracteres
  de cada respuesta, y la de un `map_decidir` que no acciona lleva detrás el inventario (263) con las filas por su
  texto. Es la conversación con Luna, no lo que viaja a Jev. Hallazgo para el dueño, junto a las `mano:`.
- **2026-09-22, fase 7.** El campo `ElTramo.Paso.Etiqueta` lleva desde esta fase **el nombre para contar**, no
  siempre la etiqueta: el tramo lo escribe en «paso k:», en la cuenta, en «última puerta» y en «la misma puerta
  tres veces», y pulsa por `Selector`, así que no necesitaba el texto para nada. `ElTramo.cs` no se toca; el
  nombre del campo queda viejo y se dice aquí y en el comentario de `UnPasoDecidido`.
- **2026-09-22, fase 10.** Dos vías de la política que quedan **sin tocar**, las dos **(D, sin medir)** y las dos sin
  juez posible sin leer UIA de verdad (el lector de `SurfaceMapTools` no se inyecta): (a) `PuertasDeAhora` lee `aqui`
  ...
  se lee?).
- **2026-09-22, fase 10.** `InstanciarSkill.cs:46` corta una skill enseñada con **solo** `PuertasPeligrosas.EsPeligrosa`:
  la misma clase que el hallazgo 5 (una lista de dos). No se toca aquí: es la promesa 126 (spec 009) y otra pregunta
  —reproducir lo que una persona enseñó, no lo que un modelo eligió—. Hallazgo para quien tenga esa zona.
- **2026-09-22, fase 10.** Consecuencia de vetar también con `SafeToClick.EsDestructivo`: `map_decidir` y el tramo dejan
  de pulsar «Aceptar», «Confirmar», «Permitir», «Instalar», «Compartir», «Subir»… En un diálogo «¿Desea eliminar…?»
  «Aceptar» es eliminar, así que es fallar cerrado; pero en SAP y en Windows «Aceptar» es también el «OK» de cualquier
  diálogo, y el tramo parará ahí más a menudo. Es medible en el nivel 4 (cuántas veces para por «Aceptar») y, si estorba,
  se discute la lista por spec, no en silencio.
- **2026-09-22, fase 10.** `SinTextoDeFilas` sustituye lo que va **entre «»**: la mano real cita así la etiqueta y el
  selector en los mensajes que se conocen (`PulsarSegunElNucleo`, homónimos, «Vivo aquí»). Un motivo de la mano de SAP
  que llevara el selector de una fila **sin comillas** no se limpiaría **(D, no hay ninguno medido)**. Y dos filas con la
  misma etiqueta quedan nombradas por la primera: no filtra texto, pero la cuenta diría «fila 2» de las dos.
...
| — | promesas 345–349 (y 350, 386–388) | (no era una refutación; 2026-09-22, tras la fase 4) el merge de `origin/main` trajo `6554aa5` y `043addc`, que gastaron 345–349 | **(M)** `git grep` sobre 49 refs `origin/*` y las ramas locales: 389–399 libres; 350 solo en esta rama | **345→389, 346→390, 347→391, 348→392, 349→393**, sin reciclar ninguna: en el registro y en los 5 `Pendiente` de `Contrato.cs`, en los 3 comentarios del ayudante `RespuestaChoice`/`IdNinguna`, en los 17 comentarios de `Decision/` que las citan y en esta spec. Los enunciados no cambian. La propuesta para el documento del dueño pasa de 389+ a **394+** |
| — | el juez de la 393 llamaba a `ElegirConModelo` con `Leer(entorno)` a mano | (no era una refutación; 2026-09-22, fase 6) así no juzgaba el cableado: el interruptor podía seguir pasando `PorDefecto` —lo que hacía desde la fase 4— y la 393 salía verde con `U_DECISOR_TEXTO_VETADO` leída y sin vetar nada en producción. Es el mismo razonamiento por el que la 392 juzga por el interruptor | **(M)** con la política y `ElDecisor` ya hechos y el interruptor aún en `PorDefecto`: `CONTRATO ROTO: 9`, la 393 roja **solo** por las dos aserciones nuevas («se llamó 1», «se llamó 0») | el juez gana su parte 4 (por el interruptor, con `FabricaDeTransporte`); **el enunciado no cambia** —ya decía «más los de U_DECISOR_TEXTO_VETADO»—; y `InterruptorDelDecisor.cs:84` pasa `cfg.Politica` |
| — | el juez de la 350 no tenía filas sin número ni filas vetadas | (no era una refutación; 2026-09-22, fase 7) dos huecos que el código hizo visibles: el mapa id que viaja → id ofrecido necesita ids únicos, y dos filas sin número viajarían con el mismo; y el veto de la fase 5 escribía la etiqueta en dos salidas más que la tabla no contaba (Hallazgos) | **(M)** con el resto de la fase hecho y sin las dos líneas —mapa con la primera que llega, veto por la etiqueta—: `CONTRATO ROTO: 11`, la 350 roja **solo** por sus 5 aserciones nuevas («llamadas=1, Actuar=True» y, como elegida y como segunda mejor, la cuenta del veto y la «✋ … vetada» con «fila de prueba · enviar»); las partes 1 a 3 ya verdes | el juez gana la parte 1b (ids que chocan: no se pregunta) y la parte 4 (la fila vetada se nombra por número y tipo); **el enunciado no cambia** —ya decía «se mapea por id a la puerta ofrecida» y «el relato de map_decidir … nombra una fila por número y tipo»—; `ConJev` no pregunta si dos ofrecidas distintas viajarían con el mismo id, y `Vetada` sustituye el nombre dentro de la frase de `PorQue` |
| — | el juez de la 387 solo miraba la línea | (no era una refutación; 2026-09-22, fase 9) el enunciado dice «los tres salen en la cuenta y en la línea «decisor:», también cuando no se acciona», y la sección *La señal que se registra y no decide* pide **una** composición para las dos: sin aserción sobre la cuenta, dos frases distintas —o ninguna en la cuenta— saldrían verdes | **(M)** con solo las tres propiedades declaradas: `CONTRATO ROTO: 11`, la 387 roja por 11 aserciones, entre ellas las tres nuevas (la cuenta de `map_decidir` sin la señal, accionando y sin accionar, y sin «tokens sin medir») | el juez gana tres aserciones: el tramo de «masa5» a «tokens 312» es **idéntico** en la línea y en la cuenta, también sin accionar (0 pulsos), y la cuenta sin `usage` dice «tokens sin medir»; **el enunciado no cambia** |
| — | 390 prometía el veto con la lista de `PuertasPeligrosas` y nada sobre la decisión vetada; 393 decía «en sapgui://»; los jueces de 350, 388 y 391 salían verdes | (revisión tras la fase 9, 2026-09-22: siete hallazgos, seis ciertos —el 2 y el 6 eran el mismo—) el relato de 350 llevaba el texto de la fila por la cuenta de la mano y su juez usaba una mano que no la repetía; una clave repetida apagaba en silencio la suma y el máximo; «ninguna» se decía con la confianza y el juez ponía las dos a 0,80; SAP visto por UIA viajaba; el veto no veía «Send», «Delete», «Comprar ahora», «Pagar»; y la decisión vetada seguía con `Actuar=true` y la línea «ACCIONA» | **(M)** cada uno rojo antes de su línea: `CONTRATO ROTO: 26`, rojas exactamente 388, 390, 391, 393 y 350 (fase 10) | los jueces ganan sus casos (tabla *Lo que los jueces ganaron en la fase 10*); **los enunciados de 390 y 393 se amplían** —390: «o destructiva —send, delete, comprar, pagar: la lista de SafeToClick.EsDestructivo—» y «la decisión vetada deja de decir que acciona —la línea «decisor:» no dice ACCIONA, dice que está vetada—»; 393: «en SAP —sapgui://, o su ventana vista por UIA (uia://sap…)—»—, porque sin eso los jueces juzgarían lo que el enunciado no dice. Los de 350, 388 y 391 ya lo decían («el relato de map_decidir», «exactamente las que viajaron», «con su probabilidad») y no cambian. Ningún número nuevo |
