...
números: arregló dos jueces (la 209 y la 218) sin cambiar sus enunciados. La revisión «contrato r2» del mismo día
tampoco: apretó tres (la 209, la 211 y la 47) con los enunciados intactos, y deja sin usar los números que tenía
reservados (grafo 240 en adelante, voz 70 en adelante).
...
| 211 | `TurnosSinMarca` por nombre con silencio de 1500 y reloj inyectado. Una llamada pedida queda en curso (`LlamadasEnCurso`) y sujeta el turno 6000 ms. Tras `Devuelta`, a 1000 ms no cierra y a 1500 sí, una vez. Con dos llamadas y una devuelta sigue sin cerrar. Una devuelta **antes** de oírse no queda en curso. Un trozo de audio con pico 1152 (la palabra más floja medida) sujeta el turno aunque la transcripción terminara 3000 ms antes; uno con pico 45 (el silencio medido) no lo retrasa; sonido sin nada dicho no abre nada que cerrar. **Y su cableado**: en una `ConversacionEnVivo` con GPT-Live, un `self_mute` que el contrato sujeta dentro de `Autocontrol` no deja cerrar a los 3000 ms. Al soltarlo, la conversación se lo devuelve al marcador, y cierra a 2100 ms de la devolución, no a 1500. **Y la voz que suena, también en la conversación** (revisión r2, 2026-09-13; hasta entonces solo se juzgaba sobre un marcador suelto): con el altavoz apartado por reflexión (`_silencioHastaMs` en el futuro, así `Reaccionar` tira el audio sin tocar el dispositivo), por `Procesar` llegan un `session.output_transcript.delta` y 3000 ms de `session.output_audio.delta` con pico 1152; mientras llegan no hay `Cerro`, a 1500 ms del último no, y a 2100 ms sí, uno | el marcador no registra la llamada (T1); tocar cerrar la ignora (T2); devolver no cuenta como actividad (T3); la voz que suena no sujeta (T4); el umbral a 0 (T5); una devuelta antes de oírse se queda en curso (T6); **`EjecutarAsync` no devuelve (T7)**; **la conversación no le da al marcador los `Hecho.Suena` (R2, verde con la 211 de antes: la guarda quedaba muerta)**; el umbral inalcanzable, ahora también por la conversación (S211h) |
...
- **Los 2000 ms se juzgan al milisegundo desde la revisión r2, y lo medido ese día los deja al borde.** La 209 exige
  no cerrar a 1999 ms y cerrar a 2000, en la regla y en el marcador que construye la app. Eso congela la decisión;
  no la hace buena. La sonda de los turnos del 2026-09-13 midió un hueco de **2009 ms** entre devolver una herramienta
  y lo siguiente que dijo Ü (1 de 6; los otros, 1566–1733 ms), y con 2000 ms ese turno se cerró a mitad de la tarea.
  Subirlo retrasa `Cerro`, «Ü dijo» y `DijoElUsuario`, y no se decide en esta revisión: **se decide con el dueño y se
  cambia en la 209**, que es para lo que ahora está.
- **La voz que suena se juzga también en la conversación (la 211, desde la revisión r2), con la hora a la que LLEGA el
  audio.** Lo que el marcador no ve es cuándo sale del altavoz: si la cola local va por detrás de lo recibido, el turno
  puede cerrarse con Ü todavía sonando. Sin medir en `U.exe`.
- **Los prefijos de la 47 se congelan letra por letra sin que la sonda los haya visto decidir nada.** El 2026-09-13,
  sin «CAMBIO DE MODO…» la voz también asintió (3 de 3) y con ese prefijo al volver también delegó (2 de 2). Se
  congelan porque son los textos con los que se midió todo lo demás y porque el append del aprendiz va al tope de 500
  fichas (ver «El append del aprendiz va al límite»): cambiarlos pasa a ser una conversación sobre el contrato.
...
**Revisión «contrato r2»** (2026-09-13, sobre `c5104c4`, rama `jero/voz-gpt-live-r2-contrato`). Tres jueces que no juzgaban
lo que decía su fila de «Con qué se juzga». Los tres se reprodujeron con un sabotaje de una línea sobre la 209, la 211 y
la 47 de antes (diff `1 1`, SHA cambiado, restaurado idéntico): **los tres dieron verde**. Se apretaron los jueces **sin
tocar enunciados ni código**, y **sin números nuevos**: cada enunciado ya prometía lo que faltaba juzgar, y la precedencia
es la del cierre (la 209 y la 218 arregladas sin número). Los números 240+ del grafo y 70+ de la voz quedan sin usar.

| # | Sev. | Hallazgo | ¿Real? | Qué se hizo | Juez hoy |
|---|---|---|---|---|---|
| C2-1 | media | La 209 no fijaba los 2000 ms: cualquier silencio por defecto entre 1701 y 2000 pasaba | real: con `SilencioPorDefectoMs = 1705`, **CONTRATO INTACTO, exit 0** (R1). Y contra el servidor, ese día: huecos de devolver a hablar Ü de 1566–1733 ms y **uno de 2009** | la 209 exige no cerrar a 1999 ms y cerrar a 2000, en la regla y en la conversación | 209; R1, S209j y S209k rojas. **Los 2000 ms, al borde: ver «Límites»** |
| C2-2 | media | Nadie juzgaba que la conversación le pase al marcador la voz que suena | real: con `hecho is not Hecho.Suena &&` delante de `Oye`, **CONTRATO INTACTO, exit 0** (R2). Contra el servidor: en respuestas largas la voz siguió llegando 3089, 2667 y 2406 ms sin transcripción nueva, y sin la guarda el turno se cerró con Ü sonando en 3 de 6 | la 211 lo juzga por `Procesar`, con el altavoz apartado | 211; R2 y S211h rojas |
| C2-3 | media | La fila de la 47 decía juzgar `AlCambiarDeModo` y `AlVolver`, y el contrato solo miraba `Contains` | real: sin el prefijo (R3a) y con el prefijo equivocado al volver (R3b), **VOZ ÍNTEGRA, exit 0**. Contra el servidor, ese día, el prefijo no decidió nada: sin él la voz asintió 3 de 3; con el equivocado, volvió a delegar 2 de 2 | la 47 compara letra por letra con los textos medidos, copiados en el contrato | 47; R3a, R3b, S47e y S47f rojas |
...
- **2026-09-13 (revisión «contrato r2»: tres jueces que no juzgaban su fila, y lo que midió el servidor con crédito).**
  - **Con crédito otra vez**: `sonda-credito.ps1` contestó `session.started` a 545 ms (11:09). Las sondas de este día
    están fuera del repo, en `scratchpad\r2-contrato\` (`sonda-adelanto.ps1`, `persona\lanzar.ps1`, y
    `turnos\sonda-turnos.ps1` repetida).
  - **Los tres hallazgos, reproducidos antes de tocar nada** (ver «Sabotajes»): R1, R2, R3a y R3b, verdes con los jueces
    de `c5104c4`.
  - **El silencio de 2000 ms, vuelto a medir** (`sonda-turnos.ps1`, cinco corridas: frases con pausas, herramientas de
    2500 y 300 ms; t1–t3 con otras sondas en paralelo, t4 y t5 solas). De devolver una herramienta a lo siguiente que
    dice Ü: **1566, 1587, 1617, 1622, 1733 y 2009 ms**. El de 2009 (t4, sola) supera los 2000: la regla reproducida
    sobre esa línea cierra el turno 9 ms antes de «Ya está», a mitad de la tarea. Con los cuatro de ayer (1658–1707)
    son 10 huecos: 1 por encima de 2000 y 3 por encima de 1705. **Los 2000 ms quedan al borde**, y no se cambian aquí
    (ver «Límites»). Medido de paso, sin arreglar: la transcripción de la última palabra del usuario llegó **4,7 s** (t1)
    y **~24 s** (t5) después de decirla, con el delegado ya trabajando; esa frase se parte con 2000, con 2500 y con
    3000 ms.
  - **La voz que suena, contra el servidor** (`sonda-adelanto.ps1`: un dictado de 26 s, dos respuestas largas sin
    herramienta y tres con `map_look` y un resultado largo; las dos primeras con cinco sondas en paralelo). Se mide lo
    que llega: cuánto tramo de audio con voz (pico > 1000) llega sin transcripción nueva, y la regla reproducida con y
    sin la guarda.
    - Dictado: 1165 ms como mucho; la última palabra, 499 ms antes que la última voz. Sin la guarda no cierra antes.
    - Respuestas largas: **3089, 2667, 2406, 1969 y 1001 ms**. Sin la guarda, el turno se cerró con voz llegando
      después en 3 de las 6 corridas (144, 108 y 5 deltas después del cierre) y al borde en una cuarta; con la guarda,
      en ninguna. El adelanto se midió antes solo para la última palabra (650–750 ms); entre dos frases es mayor.
  - **Los prefijos de la 47, contra el servidor** (`persona\lanzar.ps1`, diez sesiones en paralelo con la persona
    actual de la voz y el `ModoAprendiz.Instrucciones` de la rama, narrando «Ahora escribo NWP1…»):
    - con `AlCambiarDeModo` delante, asintió 3 de 3 («Ajá», «Uhum»); **sin él, también 3 de 3**, ninguna afirmó una acción;
    - al volver con `AlVolver`, delegó `map_look` 2 de 2; **con `AlCambiarDeModo` en su lugar, también 2 de 2**.
    - Lo que cambia la voz es el append de las reglas, no su prefijo. Se congelan igual (ver «Límites»), y el
      comentario de `ProtocoloGptLive` lo dice.
    - **La transcripción fantasma, más de lo medido**: en 6 de las 10 sesiones llegó ~2 s tras el append un trozo del
      usuario que nadie dijo, y en dos de ellas es **un pedazo del propio append** («s ves?», «ves?»),», de «¿ves
      esto?»). Ayer fueron 3 de 5. Sigue sin decidir (ver «Límites»).
  - **Clase de error contada**: 3 jueces juzgaban menos de lo que decía su fila (209, 211 y 47), los tres arreglados.
    Otros con la misma forma —una fila que nombra un valor o un texto que el contrato no compara— **no se buscaron** en
    esta revisión; el único que ya estaba dicho es `SabeVolver` (C7).
...
**La revisión «contrato r2» (2026-09-13, `jero/voz-gpt-live-r2-contrato` sobre `c5104c4`).** Guion `sabotea.ps1`,
`sabotajes-hallazgos.json` y `sabotajes-nuevos.json`, en `scratchpad\r2-contrato\`: bytes en Latin-1 1:1, patrón
exactamente una vez, numstat `1 1` y SHA-256 cambiado antes de juzgar, veredicto exigido, restaurado con el SHA de
partida y el diff vacío, en los 13. SHA de partida: `TurnosSinMarca.cs` `3BA78767`, `ConversacionEnVivo.cs` `2E21BC9B`,
`ProtocoloGptLive.cs` `DDA2764E`. Logs: `sab-viejo.txt` y `sab-nuevo.txt`. El grafo con `contrato-del-grafo.ps1`, la voz
con `contrato-de-la-voz.ps1`, los dos con un `%TEMP%` propio para no pisar a otros arregladores.

Primero, **con los jueces de `c5104c4`**, los cuatro de los hallazgos:

| Id | Rotura | Veredicto con el juez de antes |
|---|---|---|
| R1 | `SilencioPorDefectoMs = 2000` → `= 1705` | **✔ 209, 211, 212 · CONTRATO INTACTO, exit 0** |
| R2 | `if (turnos.Oye(hecho))` → `if (hecho is not Hecho.Suena && turnos.Oye(hecho))` | **✔ 209, 211, 212 · CONTRATO INTACTO, exit 0** |
| R3a | `: AlCambiarDeModo + instrucciones,` → `: instrucciones,` | **✔ 47 · VOZ ÍNTEGRA, exit 0** |
| R3b | `vuelve ? AlVolver + InstruccionesDeLaVoz` → `vuelve ? AlCambiarDeModo + InstruccionesDeLaVoz` | **✔ 47 · VOZ ÍNTEGRA, exit 0** |

Con los jueces apretados y el código sin tocar: **CONTRATO INTACTO** y **VOZ ÍNTEGRA**, exit 0. Después, los nueve:

| Id | Rotura | Veredicto |
|---|---|---|
| R1 | el silencio por defecto baja a 1705 | ✘ 209 («a 1999 ms del último trozo todavía no toca cerrar…», «a 2000 ms del último trozo toca cerrar, y una sola vez», y en la conversación «cerró 1, 1 y luego 1»); ✔ 211 y 212. ROTO, exit 3 |
| S209j | la app construye el marcador con 1705 | ✘ 209 («…a 1999 ms no cierra, a 2000 ms cierra, y una sola vez… (cerró 1, 1 y luego 1)»). ROTO, exit 1 |
| S209k | la app construye el marcador con 2050 | ✘ 209 (la misma, «cerró 0, 0 y luego 1»). ROTO, exit 1 |
| R2 | la conversación no le da al marcador los `Hecho.Suena` | ✘ 211 («en la conversación, mientras llega la voz de Ü (pico 1152) no se cierra el turno… (cerró 1)» y «…a 1500 ms no cierra, a 2100 ms sí (cerró 1 y luego 1)»); ✔ 209 y 212. ROTO, exit 2 |
| S211h | `PicoDeVoz = 40000` (T4, repetido) | ✘ 211 en la regla (dos) y en la conversación (dos). ROTO, exit 4 |
| R3a | el append del aprendiz sin su prefijo | ✘ 47 («…letra por letra… (esperados 306 caracteres, llegan 200; la primera diferencia en el 0: «Eres Ü, y ahora mismo…»)»). VOZ ROTA, exit 1 |
| R3b | la vuelta con el prefijo de cambiar de modo | ✘ 47 («…detrás del prefijo medido de la vuelta… (esperados 726 caracteres, llegan 721; la primera diferencia en el 0: «CAMBIO DE MODO…»)»). VOZ ROTA, exit 1 |
| S47e | `las anteriores:\n` → `las otras:\n` en `AlCambiarDeModo` | ✘ 47 («…la primera diferencia en el 94: «otras:…»»). VOZ ROTA, exit 1 |
| S47f | `ya no manda; desde ahora` → `ya no manda, desde ahora` en `AlVolver` | ✘ 47 («esperados 726 caracteres, llegan 726; la primera diferencia en el 76…»). VOZ ROTA, exit 1 |

**S209j y S209k solo los ve la conversación**: la regla se juzga con el valor por defecto, y el marcador que construye
la app, por `Procesar`. **S211h** comprueba que el bloque nuevo de la 211 responde a la regla que dice juzgar, y no solo
a la rotura del cableado. Los textos de los prefijos van **copiados** en el contrato y no leídos por reflexión: una
reflexión compararía la constante consigo misma, y S47e y S47f quedarían verdes.
