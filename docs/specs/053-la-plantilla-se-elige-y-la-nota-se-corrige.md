...
> **Integrada el 2026-09-26** en `claude/admiring-brahmagupta-fghedd`, como base de las specs 054-055
> (Miracle Notes en Windows). Nació como **spec 015 con promesas 189-193**; al integrarla, `main` ya
> tenía otra 015 y otras 189-193, así que pasa a **spec 053 con promesas 440-444**. El commit
> `f89a64a` las cita por su número viejo.
...
Se numera desde **440**: `main` va por la 179 y **184-188 están reservadas** por la rama del collar
(spec 014), que no se toca desde aquí.

| # | Promesa | Fase |
|---|---|---|
| 440 | la plantilla con la que se graba sale de una cadena de orden fijo —lo que el médico eligió, su sugerida, la de urgencias, la abierta— y nunca de una cualquiera del catálogo | 1 |
| 441 | la sugerida del médico se guarda a su nombre y por su especialidad, y al volver a abrir la app manda sobre la de urgencias | 2 |
| 442 | **corregir una sección manda la nota ENTERA: todas las claves del snapshot, también las que quedaron vacías** | 3 |
| 443 | una nota firmada no se edita desde Windows: se ve, se sigue pudiendo mandar a SAP, y el editor no aparece | 4 |
| 444 | al re-escribir una consulta que ya existe, el espejo no manda estado ni firma: lo que el portal avanzó sobrevive a una edición hecha en Windows | 5 |
...
**La que cierra el asunto es la 442.** Mientras corregir pueda mandar media nota, las otras cuatro son
...
- **440** — `U.WindowsClient.Clinical.ReglaDeLaPlantilla.Elegir(catalogo, elegida, pin, especialidad)`,
...
- **441** — el cuerpo y la ruta de la escritura del pin, contra el `BackendDeMentira` que ya tiene el
...
- **442** — el `note_json` que se compone al guardar, sobre una nota con una sección **vacía**.
  **Falsifica**: que el payload lleve menos claves que el snapshot, o alguna de más.
- **443** — `ReglaDeLaEdicion.SePuedeEditar(estado)` sobre los cinco estados del portal.
  **Falsifica**: que `aprobada` o `exportada` contesten que sí.
- **444** — `EspejoDeConsulta.Fila(...)` con y sin fila previa. **Falsifica**: que la versión de
...
|---|---|---|---|
| **0** | ninguna — arnés | `tests/ContratoDelGrafo/Contrato.cs` | 440-444 **ROJAS**, 1..179 intactas |
| **1** | 440 | `Clinical/ReglaDeLaPlantilla.cs` (nuevo) | 440 verde |
| **2** | 441 | `Clinical/SugeridaDelMedico.cs` (nuevo), `Cuenta/SesionMiracle.cs` | 441 verde |
| **3** | 442 | `Clinical/ClinicaClient.cs` (`PUT /note`), `Clinical/Consulta.cs` | 442 verde |
| **4** | 443 | `Clinical/ReglaDeLaEdicion.cs` (nuevo) | 443 verde |
| **5** | 444 | `Clinical/EspejoDeConsulta.cs` | 444 verde |
...
| **Adendas** sobre notas firmadas | Es el camino correcto para corregir lo firmado, y es una feature entera con su tabla append-only. La 443 se limita a **no dejar editar**, que es lo que impide el daño |
...
- [ ] 440-444 verdes y 1..179 intactas (`.\scripts\contrato-del-grafo.ps1` → CONTRATO INTACTO)
- [ ] Sabotaje comprobado en la 442: si el payload deja fuera una sección vacía, la promesa se pone roja
